...
std::vector<pixel> Decode(const std::vector<uint8_t> &data, bool skip_preview) {
    ...
    if (pos + 2 > data.size()) throw std::runtime_error("Truncated file");
    ...
    if (pos >= data.size()) throw std::runtime_error("Truncated file");
    uint8_t channel_mask = data[pos];
    pos += 1;

    if (pos + 8 > data.size()) throw std::runtime_error("Truncated file");
    ...
    public:
        ...
        std::pair<bool, bool> readBit() {
            if (byte_idx >= buf.size()) return {false, false};
    ...
    for (int channel = 0; channel < 3; channel++) {
        ...
        if (pos + 4 > data.size()) throw std::runtime_error("Truncated file");
        pos += 4;
        ...
        auto readChunk = [&](size_t& pos) -> std::vector<uint8_t> {
            ...
            uint32_t len = (static_cast<uint32_t>(data[pos]) << 24) |
                           ...
                           (static_cast<uint32_t>(data[pos+3]));
            pos += 4;
            if (pos + len > data.size()) throw std::runtime_error("Truncated file");
        ...
        for (uint32_t block_y = 0; block_y < full_height; block_y += macroBlock) {
            for (uint32_t block_x = 0; block_x < full_width; block_x += macroBlock) {
                ...
                if (is_big) {
                    if (fg_idx >= fg_vals.size() || bg_idx >= bg_vals.size()) break;
                    ...
                    for (uint32_t y = 0; y < macroBlock; y++) {
                        for (uint32_t x = 0; x < macroBlock; x++) {
                            ...
                            if (channel == 0) Y_plane[idx] = value;
                            else if (channel == 1) Cb_plane[idx] = value;
                            else Cr_plane[idx] = value;
                        }
                    }
                } else {
