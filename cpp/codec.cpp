...
encodedChannel encodeChannel(std::vector<uint8_t> &plane, int stride, int w4, int h4, int fullW, int fullH, bool useMacro, int smallBlock, int macroBlock, int encQuality)
{
    ...
    {
        for (int mx = 0; my < fullW; mx += macroBlock)
        {
            ...
            else
            {
                ...
                for (int by = 0; by < macroBlock; by += smallBlock)
                {
                    for (int bx = 0; bx < macroBlock; bx += smallBlock)
                    {
                        ...
                        if (smallBlock > 1)
                            pw = &patternW;
                        auto [fg, bg] = encodeBlock(&plane, stride, mx + bx, my + by, smallBlock, smallBlock, (*pw));
                        fgVals.push_back(fg);
                        bgVals.push_back(bg);
...
std::vector<uint8_t> Encode(image img, int quality, bool bwmode)
{
    ...
    bwWriter.writeUint16(width);
    bwWriter.writeUint16(height);

    int w4 = (width / smallBlock) * smallBlock;
    int h4 = (height / smallBlock) * smallBlock;
    int fullW = (w4 / macroBlock) * macroBlock;
    int fullH = (h4 / macroBlock) * macroBlock;
    ...
    for (int i = 0; i < lol; ++i)
    {
        encodedChannel Channel = encodeChannel(planes[i], width, w4, h4, fullW, fullH, macroBlock > smallBlock, smallBlock, macroBlock, quality / 20);
